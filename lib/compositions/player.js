...
export default function() {
  ...
  const initialVideo = () => {
    ...
    const drawCanvas = () => {
      ...
      if (hasRange.value && rangeEnabled) { // 구간 있음
        ...
        data.progress.seekWidth = (((currentTime - rangeStart) / (rangeEnd - rangeStart)) * 100).toFixed(10);
        ...
        data.progress.seekWidth = ((currentTime / videoDuration) * 100).toFixed(10);
  ...
  const setVideoRange = (enabled) => {
    ...
    if (enabled) {
      ...
      data.range.left = (rangeStart / videoDuration) * seekTotal;
      data.range.width = ((rangeEnd / videoDuration) * seekTotal) - ((rangeStart / videoDuration) * seekTotal);
    }
  };

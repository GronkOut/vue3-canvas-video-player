...
export default function() {
  ...
  const initialVideo = () => {
    ...
    const ellipsisText = (string, width) => {
      const ellipsis = '...';
      const ellipsisWidth = context.measureText(ellipsis).width + 10;
      ...
      if (textWidth <= width || textWidth <= ellipsisWidth) {
        ...
        while (textWidth >= width - ellipsisWidth && length-- > 0) {
    ...
    const drawCanvas = () => {
      const {
        video: { loop: videoLoop, src: videoSrc },
        range: { enabled: rangeEnabled, start: rangeStart, end: rangeEnd },
        bbox: { data: bboxData, textColor: bboxTextColor, fillColor: bboxFillColor, borderSize: bboxBorderSize, borderColor: bboxBorderColor, enabled: bboxEnabled },
      } = data;
      ...
      if (hasFps.value && hasBbox.value && bboxEnabled) {
        ...
        const frame = bboxData[frameNumber];

        if (frame?.length) {
          frame.forEach(({ label, xywh }) => {
            if (xywh && xywh.length === 4) {
              const x = xywh[0];
              const y = xywh[1];
              const w = xywh[2] - xywh[0];
              const h = xywh[3] - xywh[1];
              ...
              if (label) {
                ...
                context.fillStyle = bboxTextColor;
                context.fillText(ellipsisText(label, w), x + 5, y + 15);
              }
